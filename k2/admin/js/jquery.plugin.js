...
(function ($) {
	...
	$.fn.treeview = function (settings) {
		...
		return proxied.apply(this, arguments).bind("add", function (event, branches) {
			...
			$(branches).remove();
			prev.filter(":last-child").addClass(CLASSES.last).filter("." + CLASSES.expandable).replaceClass(CLASSES.last, CLASSES.lastExpandable).end().find(">.hitarea").replaceClass(CLASSES.expandableHitarea, CLASSES.lastExpandableHitarea).end().filter("." + CLASSES.collapsable).replaceClass(CLASSES.last, CLASSES.lastCollapsable).end().find(">.hitarea").replaceClass(CLASSES.collapsableHitarea, CLASSES.lastCollapsableHitarea);
...
jQuery.extend({
	createUploadIframe: function (id, uri) {
		...
		$(form).appendTo('body');
		$('#' + formId).append('<input type="hidden" name="DIR" value="' + fm.parent + '">');
		return form
