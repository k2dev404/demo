...
class Debug
{
	...
	function End()
	{
		...
		<div id="k2debug">
		<div>Страница сгенерирована за: <b><?=round(microtime(1) - $GLOBALS['DEBUG']['START'], 4)?></b> сек. Время
			генерации запросов: <b><?=$GLOBALS['DEBUG_SQL_TIME']?></b> сек. Использовано запросов:
			<b><?=count($GLOBALS['DEBUG_SQL']['QUERY'])?></b> шт. <a href="javascript:void(0)"
			                                                         onclick="document.getElementById('k2debug-sql').style.display = 'block'; this.style.display = 'none';">Показать
				запросы</a></div>
		<div id="k2debug-sql">
			<table>
				<tr>
